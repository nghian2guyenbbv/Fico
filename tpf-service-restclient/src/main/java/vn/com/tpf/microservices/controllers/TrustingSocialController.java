...
public class TrustingSocialController {
	...
	public ResponseEntity<?> firstCheck(@RequestHeader("Authorization") String token, @RequestBody ObjectNode body)
			throws Exception {
	...
	@PostMapping("/trusting-social/lead/resubmit-document-info")
	@PreAuthorize("#oauth2.hasAnyScope('tpf-service-trusting-social')")
	public ResponseEntity<?> update(@RequestHeader("Authorization") String token, @RequestBody ObjectNode body)
			throws Exception {
		body.put("reference_id", UUID.randomUUID().toString());
		Map<String, Object> request = new HashMap<>();
