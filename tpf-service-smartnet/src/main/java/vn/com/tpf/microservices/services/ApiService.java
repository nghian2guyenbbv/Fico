	...
	private JsonNode uploadDocument(String url, ObjectNode data, JsonNode documentDbInfo) {
		try {
			data.put("documentUrlDownload",
					data.path("documentUrlDownload").asText().replace(urlWebViewSmartnet, urlWebViewSmartnetReplace));
	...
	public ObjectNode pushAppIdOfLeadId(Smartnet smartnet) {
		...
		try {
			...
			JsonNode encrypt = rabbitMQService.sendAndReceive("tpf-service-assets",
					Map.of("func", "pgpEncrypt", "body", Map.of("project", "smartnet", "data", dataString)));

			HttpEntity<String> requestEntity = new HttpEntity<>(encrypt.path("data").asText(), requestHeaders);

			ResponseEntity<String> responseEntity = restTemplate.exchange(urlSmartnetPushAppId, HttpMethod.POST,
					requestEntity, String.class);
						
			JsonNode decrypt = rabbitMQService.sendAndReceive("tpf-service-assets", Map.of("func", "pgpDecrypt", "body",
					Map.of("project", "smartnet", "data", responseEntity.getBody().toString())));
