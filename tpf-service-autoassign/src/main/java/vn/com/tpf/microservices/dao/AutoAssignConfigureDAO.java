...
public interface AutoAssignConfigureDAO extends JpaRepository<AutoAssignConfigure, String> {
    ...
    @Query(value = "UPDATE auto_assign.autoassign_configure " +
                    "SET total_quanlity = ?1, quanlity_in_day = ?2, priority = ?3, id_seq = nextval('auto_assign.seq_autoassign_configure'), is_assign = ?4, updated_date = current_date, create_by= ?7 " +
...
//    List<AutoAssignConfigure> getHistory(int limit, int page, String orderBy, String order);
    ...
    @Query(value = "SELECT * " +
            "FROM auto_assign.autoassign_configure ", nativeQuery = true)
    List<AutoAssignConfigure> getHistory(Pageable paging);
