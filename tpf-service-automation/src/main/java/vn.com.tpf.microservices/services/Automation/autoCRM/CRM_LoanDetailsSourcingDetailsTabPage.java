...
import vn.com.tpf.microservices.models.AutoCRM.CRM_LoanDetailsDTO;
...
@Getter
public class CRM_LoanDetailsSourcingDetailsTabPage {
    ...
    public CRM_LoanDetailsSourcingDetailsTabPage(WebDriver driver) {
        PageFactory.initElements(driver, this);
        _driver = driver;
    }

    public void setData(CRM_LoanDetailsDTO data) {
        //default branch HO CHI MINH
        if (StringUtils.isNotBlank(data.getSourcingDetails().sourcingBranch)) {
            branchElement.clear();
            branchElement.sendKeys(data.getSourcingDetails().sourcingBranch);
            ...
            for(WebElement e: branchOptionElement) {
                if(!Objects.isNull(e.getAttribute("username")) && e.getAttribute("username").equals(data.getSourcingDetails().sourcingBranch)) {
                    e.click();
        ...
        Utilities.chooseDropdownValue(data.getSourcingDetails().sourcingChannel, channelOptionElement);

        if(!Objects.isNull(data.getSourcingDetails().applicationNumber)){
            applicationFormNumberElement.sendKeys(data.getSourcingDetails().applicationNumber);
        }

        loanApplicationTypeElement.click();
        await("channelOptionElement loading timeout").atMost(Constant.TIME_OUT_S, TimeUnit.SECONDS)
                .until(() -> loanApplicationTypeOptionElement.size() > 0);
        Utilities.chooseDropdownValue(data.getSourcingDetails().loanApplicationType, loanApplicationTypeOptionElement);
        ...
        Utilities.chooseDropdownValue(data.getSourcingDetails().productCode, productNameOptionElement);

        schemeElement.click();
        await("schemeOptionElement loading timeout").atMost(Constant.TIME_OUT_S, TimeUnit.SECONDS)
                .until(() -> schemeOptionElement.size() > 0);
        Utilities.chooseDropdownValue(data.getSourcingDetails().schemeCode, schemeOptionElement);

        Utilities.checkValueSendkey(data.getSourcingDetails().loanAmountRequested,loanAmountElement);
        ...
        loanTermElement.sendKeys(data.getSourcingDetails().requestedTenure);
        Utilities.captureScreenShot(_driver);
        ...
        Utilities.chooseDropdownValue(data.getSourcingDetails().saleAgentCode, salesAgentCodeOptionElement);
    }
