...
@Getter
public class LD_ApplicationInfoPersonalTab {
    ...
    @FindBy(how = How.ID, using = "phoneNumberList1_phoneNumber")
    ...
    @FindBy(how = How.ID, using = "save_cust_address")
    ...
    public LD_ApplicationInfoPersonalTab(WebDriver driver) {
    ...
    public void setValue(ApplicationInfoDTO applicationInfoDTO) {
        this.genderSelectElement.click();
        await("genderSelectOptionElement loading timeout").atMost(Constant.TIME_OUT_S, TimeUnit.SECONDS)
                .until(() -> genderSelectOptionElement.size() > 0);
...
//        this.btnLoadAddressElementElement.click();
    ...
    public void setAddressValue(List<AddressDTO> datas) throws JsonParseException, JsonMappingException, IOException, InterruptedException {
        ...
        for (AddressDTO data : datas) {
            ...
            await("addressDivElement display Timeout!").atMost(120, TimeUnit.SECONDS)
                    .until(() -> addressDivElement.isDisplayed());
            actions.moveToElement(mobilePhoneNumberElement).click().build().perform();
